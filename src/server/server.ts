import ws, {TemplatedApp} from 'uWebSockets.js';
...
const getApp = (ssl: boolean): TemplatedApp => {
  if (ssl) {
    return ws.SSLApp({
...
export const initApp = async (ssl: boolean): Promise<TemplatedApp> => {
  const wsApp = getApp(ssl);
  ...
  wsApp.ws('/websocket', {
                  ...
                  if (key !== id) {
                    ...
                    for (let i = 0; i < arrcons.length; i++) {
                      ...
                      if (consumer_video) {
                        ...
                        while (i--) {
                          if (
                            consumer_video[i].producerId == producer_video.id
                          ) {
                  ...
                  if (key !== id) {
                    ...
                    for (let i = 0; i < arrcons.length; i++) {
                      ...
                      if (consumer_audio) {
                        ...
                        while (i--) {
                          if (
                            consumer_audio[i].producerId == producer_audio.id
                          ) {
    ...
    message: (ws, message, isBinary) => {
      ...
      let msgo: Message;
      try {
        msgo = JSON.parse(msg) as Message;
      } catch (err) {
